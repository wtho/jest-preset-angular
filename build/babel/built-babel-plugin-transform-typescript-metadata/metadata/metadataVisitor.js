...
function metadataVisitor(classPath, path) {
    var field = path.node;
    var classNode = classPath.node;
    switch (field.type) {
        case 'ClassMethod':
            var decorators = field.kind === 'constructor' ? classNode.decorators : field.decorators;
            if (!decorators || decorators.length === 0)
                return;
            decorators.push(createMetadataDesignDecorator('design:type', _core.types.identifier('Function')));
            decorators.push(createMetadataDesignDecorator('design:paramtypes', _core.types.arrayExpression(field.params.map(function (param) { return (0, _serializeType.serializeType)(classPath, param); })))); // decorators!.push(

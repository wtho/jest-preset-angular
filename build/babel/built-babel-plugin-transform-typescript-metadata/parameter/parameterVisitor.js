 ...
 */
function makeParamDecorator(paramIdx, decExpr, isConstructor) {
    ...
    [_core.types.identifier('target'), _core.types.identifier('key')], _core.types.blockStatement([_core.types.returnStatement(_core.types.callExpression(decExpr, [_core.types.identifier('target'), _core.types.identifier(isConstructor ? 'undefined' : 'key'), _core.types.numericLiteral(paramIdx)]))])));
}
function parameterVisitor(classPath, path) {
    ...
    if (path.node.key.type !== 'Identifier')
        return;
    var methodPath = path;
    var params = methodPath.get('params') || [];
    params.slice().forEach(function (param) {
        var identifier = param.node.type === 'Identifier' ? param.node : param.node.type === 'TSParameterProperty' && param.node.parameter.type === 'Identifier' ? param.node.parameter : null;
        if (identifier == null)
            return;
        var resultantDecorator;
        (param.node.decorators || []).slice().forEach(function (decorator) {
            if (methodPath.node.kind === 'constructor') {
                resultantDecorator = makeParamDecorator(param.key, decorator.expression, true);
                ...
                resultantDecorator = makeParamDecorator(param.key, decorator.expression, false);
